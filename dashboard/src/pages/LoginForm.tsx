...
export default function LoginForm({ role }: LoginFormProps) {
  ...
  const roleName = role === 'customer' ? 'Customer' : role === 'owner' ? 'Owner' : 'Admin';

  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-10">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-serif text-gray-900 mb-2 tracking-tight">{roleName} Sign In</h1>
        ...
        {role !== 'admin' && (
          <p className="mt-8 text-center text-sm text-gray-500">
            ...
            <Link to={`/signup?as=${role}`} className="text-[var(--color-terracotta-600)] font-medium underline">
              Create one here
            </Link>
          </p>
        )}
